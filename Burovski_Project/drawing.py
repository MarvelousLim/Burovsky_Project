...
def draw(ode_sol, t, energy, m, L):
    """Модуль drawing отвечает за рисование четырех анимаций: реального движения маятника и 
    трех различных фазовых диаграмм. Функция draw(ode_sol, t, energy, m, L) принимает на вход 
    пять аргументов, где ode_sol должна содержать массив (2, n) решений дифференциального уравнения 
    (например, непосредственный вывод solve_ivp или ode_int), t - массив отметок во времени 
    (к которым относятся точки из ode_sol), energy - массив размера n предпосчитанных энергия 
